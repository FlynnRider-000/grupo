...
$('body').on('click', '.swr-grupo .aside > .content > .addmore > span', function(e) {
    ...
    if (i == 'uploadfile') {
        ...
        $('.swr-grupo .lside > .tabs > ul > li[act="'+i+'"]').trigger('click');
    } else {
        menuclick(c, i);
    }
});
        ...
        if (!$('.swr-grupo .panel > .textbox').hasClass('slideOutDown')) {
            ...
            if (blob !== null) {
                ...
                reader.onload = function(event) {
                    ...
                    var s = 'loadmsg(data);';
                    var f = grlv = '';
                    ajxx($('.pastescreen'), data, s, e, f, 'screenshotz', '');
                };
...
$("body").on("keyup", ".swr-grupo .panel > .searchbar input", function(e) {
    if (e.which == 13) {
        ...
        if (search != "") {
            ...
            ajxx($(this), data, s, e, f, 'searchmsgs', '');
        }
    }
});
    ...
    if (!$(".swr-grupo .panel > .room > .msgs").hasClass('nooutscroll')) {
        ...
        if (!$(".swr-grupo .panel > .textbox").hasClass('disabled') && $('.swr-grupo .panel').attr('noscroll') != 1) {
            ...
            if (scrollTop <= 0) {
                ...
                ajxx($(this), data, s, e, f, 'scrollmsgs', '');
            }
        }
    ...
    if (nl == 0) {
        return (text || "").replace(
            ...
            function(match, space, url) {
                ...
                var emailnew = email.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/gi);
                if (emailnew != null) {
...
function htmlescaper(unsafe_str) {
    return unsafe_str
    .replace(/&/g, '&')
    .replace(/</g, '<')
    .replace(/>/g, '>')
...
function shwrdmre(str, len, nl, ej) {
    ...
    if (maxLength !== "") {
        if ($.trim(str).length > maxLength) {
            ...
            if (len == 0) {
                $shrt = $shrt.replace(/(?:\r\n|\r|\n)/g, '<br> ');
                $shrt = '<i class="shortmsg">'+gremojibuild(escapeHtml($shrt), ej)+'</i><i class="readmore">'+$(".gphrases > .readmore").text()+'</i>';
