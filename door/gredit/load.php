...
function gr_edit() {
    ...
    if ($arg[0] === 'group') {
            ...
            if (!empty($arg[1]['name'])) {
                ...
                if ($cr == 0) {
                    ...
                    if (isset($GLOBALS["roles"]['groups'][14])) {
                        if (isset($arg[1]['visibility']) && !empty($arg[1]['visibility']) && $nmchk[0]['v3'] != $arg[1]['visibility']) {
                            if ($arg[1]['visibility'] != 'visible') {
                    ...
                    if (isset($arg[1]['delpass']) && $arg[1]['delpass'] == 1) {
                        gr_data('u', 'v2', 'type,id', '0', 'group', $arg[1]['id']);
                        $dt = array();
                        $dt['id'] = $arg[1]["id"];
        ...
        if (isset($GLOBALS["roles"]['users'][2]) || $arg[1]['id'] == $GLOBALS["user"]['id']) {
            ...
            $oldemail = 0;
            if (usr('Grupo', 'select', $arg[1]['id'])['email'] == $arg[1]['email']) {
                $oldemail = 1;
            }
            if (usr('Grupo', 'alter', 'email', $_POST['email'], $arg[1]['id']) || $oldemail == 1) {
